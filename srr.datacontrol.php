<?php
	...
	switch($_REQUEST['mod']) {
		...
		case "addItem":
			...
			if($_POST['expiry'] != '') { $expr = $mydb->formatDate($_POST['expiry']); } else { $expr = ''; }

			list($isE) = $mydb->getArray("select count(*) from srr_details where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]' and lot_no = '$_POST[lot_no]' and expiry = '$expr';");
			if($isE > 0) {
				$s = "update ignore srr_details set qty = qty + ".$mydb->formatDigit($_POST['qty']).", amount = amount + ".$mydb->formatDigit($_POST['amount'])." where trace_no = '$_POST[trace_no]' and item_code = '$_POST[item]' and branch = '$_SESSION[branchid]' and lot_no = '$_POST[lot_no]' and expiry = '$expr';";
			} else {
				$s = "insert ignore into srr_details (branch,srr_no,item_code,description,qty,unit,cost,amount,lot_no,expiry,trace_no) values ('$_SESSION[branchid]','$_POST[srr_no]','$_POST[item]','".$mydb->escapeString(htmlentities($_POST['description']))."','".$mydb->formatDigit($_POST['qty'])."','$_POST[unit]','".$mydb->formatDigit($_POST['cost'])."','".$mydb->formatDigit($_POST['amount'])."','$_POST[lot_no]','$expr','$_POST[trace_no]');";
			}
		...
		case "retrieveLine":
			echo json_encode($mydb->getArray("select *,format(cost,2) as ucost, format(amount,2) as amt, if(expiry!='0000-00-00',date_format(expiry,'%m/%d/%Y'),'') as expr from srr_details where line_id = '$_POST[lid]';"));
		break;
		...
		case "finalizeSRR":
			...
			$iquery = $mydb->dbquery("SELECT a.branch, a.srr_no AS doc_no, a.srr_date AS doc_date, received_from AS customer, b.item_code, b.unit, b.qty, b.lot_no, b.expiry FROM srr_header a INNER JOIN srr_details b ON a.srr_no = b.srr_no AND a.branch = b.branch WHERE a.srr_no = '$_POST[srr_no]' AND a.branch = '$_SESSION[branchid]';");
			while($ibook = $iquery->fetch_array(MYSQLI_BOTH)) {
				$mydb->dbquery("INSERT IGNORE INTO ibook (doc_no,doc_date,doc_type,doc_branch,cname,item_code,uom,inbound,lot_no,expiry,posted_by,posted_on) VALUES ('$ibook[doc_no]','$ibook[doc_date]','SRR','$ibook[branch]','".$mydb->escapeString($ibook['customer'])."','$ibook[item_code]','$ibook[unit]','$ibook[qty]','$ibook[lot_no]','$ibook[expiry]','$_SESSION[userid]',now());");
			}
		...
		case "retrieve":
			...
			$srrd = $mydb->dbquery("SELECT line_id as id, description, item_code, unit, if(expiry!='0000-00-00',date_format(expiry,'%m/%d/%y'),'') as expr, lot_no, qty, cost, amount FROM srr_details where trace_no = '$_REQUEST[trace_no]';");
